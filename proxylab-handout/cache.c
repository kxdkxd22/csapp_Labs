...
void init_cache(){
	...
	for(;i<TYPES;i++){
		...
		caches[i].cacheobjs = (cache_block *)malloc(sizeof(cache_block)*cache_cnt[i]);
		cache_block *j = caches[i].cacheobjs;
		int k;
		for(k = 0;k < cache_cnt[i];k++,j++){
			j->time = 0;
			j->datasize = 0;
...
int read_cache(char *url,int fd){
	...
	for(; tar < TYPES; tar++){
		...
		for(i=0;i<cur.size;i++,p++){
			if(p->time!=0&&strcmp(p->url,url)==0) break; 
		}
		if(i < cur.size) break;
	}

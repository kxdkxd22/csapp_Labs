...
/*void *mm_malloc(size_t size)
{
	...
	if(segregated_free_lists[listnumber]!=NULL){
                        ptr = segregated_free_lists[listnumber];
                        while((ptr!=NULL)&&((size>GET_SIZE(HDRP(ptr))))){
                                ptr=PRED(ptr);
                        }
...
/*	void *new_block = ptr;
	...
	se if(!GET_ALLOC(HDRP(NEXT_BLKP(ptr)))||!GET_SIZE(HDRP(NEXT_BLKP(ptr)))){
